...
async def create_invite_by_student(students, general_channel_id, message):
    db = get_db()
    general_channel = next((channel for channel in message.guild.channels if str(channel.id) == general_channel_id), None)
    for student in students:
        invite = await general_channel.create_invite(max_uses=1)
        # Debug
        print(f"Invitation to {student['last_name']} {student['first_name']} : {invite}")
        db[invite.code] = {
            'mail': student['mail'],
...
# async def assign_role_on_join(student):
#     db = get_db()
